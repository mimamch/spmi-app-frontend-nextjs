...
export default function GrafikSub8BagianD1() {
  const [data, setdata] = useState({});
  const getData = async () => {
    try {
      const result = await axios.get(
        `${process.env.NEXT_PUBLIC_API_ENDPOINT}/grafik/sub8/bag8D1`
      );
  ...
  return (
    <div className="card shadow mb-4">
      <div className="card-body mb-3">
        <h4 className="card-title p-2 text-dark ">Substandar 8 Bagian D1 </h4>
        <div className="row justify-content-around">
          ...
          <div className="col-md-3 ">
            <h5 className="card-title p-2 text-dark mx-5 my-5">Waktu Tunggu</h5>
            <Bar
              options={{
                plugins: {
                  legend: {
              ...
              data={{
                // labels: Object.keys(data.jumlahLulusanDenganWaktuTunggu || {}),
                labels: ["< 6 Bulan", "6 Bulan ≤ WT ≤ 18 Bulan", "> 18 Bulan"],
                datasets: [
                  {
                    label: "#",
                    data: Object.values(
                      data.jumlahLulusanDenganWaktuTunggu || {}
                    ),
                    backgroundColor: [
                      "rgba(255, 99, 132, 0.2)",
                      "rgba(54, 162, 235, 0.2)",
